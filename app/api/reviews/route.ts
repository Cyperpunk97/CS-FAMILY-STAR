...
export async function GET(request: Request) {
  try {
    ...
    // The in-memory store holds seeded demo reviews written under students' names.
    // Serving those when the real database is merely unreachable would present
    // fabricated reviews as genuine ones, so a configured-but-failing read is an error.
    if (error) {
      console.error('Supabase review fetch failed:', error.message);
      return NextResponse.json(
        { error: 'Reviews are temporarily unavailable. Please try again in a moment.' },
...
export async function POST(request: Request) {
  try {
    ...
    // Below this point the database is configured and was reachable enough to answer.
    // Falling back to the in-memory store here would be a lie: it returned 201, the
    // student saw "posted", and the review lived in one serverless instance's memory
    // until the next cold start. A write that did not persist must say so.
    if (error) {
      console.error('Supabase insert failed:', error.message);
      return NextResponse.json(
        { error: 'Your review could not be saved right now. Please try again in a moment.' },
        { status: 503, headers: noStore }
      );
    }

    if (!data || data.length === 0) {
      // An empty result with no error means RLS accepted the insert but returned no
      // row — usually a missing select policy. Either way we cannot confirm the write.
      console.error('Supabase insert returned no row; check the select policy on reviews.');
      return NextResponse.json(
        { error: 'Your review could not be confirmed. Please try again in a moment.' },
        { status: 503, headers: noStore }
      );
    }
