  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      if (file) {
        ...
        // If the upload failed, post the review without the photo. This used to fall
        // back to a `data:` URL, which the server rejects outright (it only accepts
        // URLs in our own storage bucket) — so a failed upload lost the whole review,
        // rating and comment included. The words are worth more than the picture.
        if (!uploadedUrl) {
          setPhotoWarning('Your photo could not be uploaded, so the review was posted without it.');
        }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 rounded-2xl bg-surface p-4">
      ...
      <div>
        ...
        />
        <p className="mt-1 text-end text-xs text-ink-faint">
      ...
      <div>
        ...
        {venue.signatureDish && !recommendedDish && (
          <button
            ...
            className="mt-1.5 inline-flex items-center gap-1 text-xs font-medium text-brand-700 hover:text-brand-900 transition"
          >
      ...
      {photoWarning && (
        <p
          role="status"
          className="flex items-start gap-2 rounded-xl bg-amber-50 p-3 text-xs font-medium text-amber-900 ring-1 ring-amber-200"
        >
          <AlertCircle className="mt-px h-4 w-4 shrink-0" aria-hidden="true" />
          {photoWarning}
        </p>
