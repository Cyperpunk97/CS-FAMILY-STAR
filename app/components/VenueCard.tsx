      ...
      <div className="flex min-w-0 flex-1 items-center gap-3 py-3 ps-3 text-start">
        ...
        <span className="min-w-0 flex-1">
          ...
          {venue.reviewCount > 0 && venue.averageRating >= 4 && venue.topDishes && venue.topDishes.length > 0 && (
            <span className="mt-1 flex items-center gap-1.5 truncate text-xs text-ink-soft">
              <span className="shrink-0 rounded-md bg-amber-50 px-1.5 py-0.5 text-xs font-bold text-amber-800 border border-amber-200/60">
                {t('venue.mustTry')}
              </span>
      ...
      <div className="relative z-10 flex shrink-0 items-center gap-0.5">
        ...
        >
          ...
          {copied && (
            <span
              role="status"
              className="pointer-events-none absolute -top-7 start-1/2 -translate-x-1/2 whitespace-nowrap rounded-md bg-ink px-1.5 py-0.5 text-xs font-bold text-white shadow-sm animate-fade-in"
            >
              {t('venue.copied')}
            </span>
          )}
