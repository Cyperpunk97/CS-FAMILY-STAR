...
export default function AddMemoryModal({
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Photos go to Supabase Storage and only the URL is sent to the API, exactly as
    // review photos do. Reading the file into a base64 `data:` URL instead meant the
    // whole image was posted as JSON and held in the server's memory.
    setErrorMessage('');
    ...
    try {
      ...
      const path = `memories/${unique}.${prepared.extension}`;
      ...
      if (uploadError) {
        setErrorMessage('That photo could not be uploaded. You can still post without it.');
        return;
      }

      setPhotoUrl(supabase.storage.from('review-images').getPublicUrl(path).data.publicUrl);
    } catch (err) {
      setErrorMessage(err instanceof Error ? err.message : 'Could not read that image.');
    } finally {
      ...
      <form onSubmit={handleSubmit} className="space-y-4 pt-1">
        ...
        <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
          ...
          <div>
            ...
            <div className="relative mt-1">
              <MapPin className="pointer-events-none absolute start-2.5 top-2.5 h-3.5 w-3.5 text-amber-600" />
              <input
                ...
                className="w-full rounded-xl border border-hairline bg-surface ps-8 pe-3 py-2 text-xs text-ink placeholder:text-ink-faint focus:border-amber-500 focus:bg-white focus:outline-none"
              />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-3">
          <div className="sm:col-span-1">
            ...
            <div className="relative mt-1">
              <User className="pointer-events-none absolute start-2.5 top-2.5 h-3.5 w-3.5 text-ink-faint" />
              <input
                id={`${formId}-author`}
                type="text"
                required
          ...
          <div className="sm:col-span-1">
            ...
            <div className="relative mt-1">
              <Calendar className="pointer-events-none absolute start-2.5 top-2.5 h-3.5 w-3.5 text-ink-faint" />
              <input
                id={`${formId}-date`}
                type="date"
                value={date}
                ...
                className="w-full rounded-xl border border-hairline bg-surface ps-8 pe-3 py-2 text-xs text-ink focus:border-amber-500 focus:bg-white focus:outline-none"
              />
            </div>
          </div>
        </div>
            ...
            <div className="mt-2.5 flex items-center gap-3">
              <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-lg border-2 border-white shadow-md bg-white">
                ...
                <button
                  type="button"
                  onClick={() => setPhotoUrl('')}
                  className="absolute end-1 top-1 flex h-9 w-9 items-center justify-center rounded-full bg-ink/70 text-white hover:bg-ink"
                  title="Remove photo"
                >
                  <X className="h-3 w-3" />
                </button>
            ...
            <div className="mt-2 space-y-2">
              ...
              >
                ...
                <span className="text-xs font-bold text-amber-950">
                  {uploadingPhoto ? 'Uploading your photo…' : 'Click to upload your outing photo'}
                </span>
                <span className="text-xs text-ink-faint">PNG, JPG or WebP up to 5MB</span>
                <input
                  ref={fileInputRef}
                  type="file"
        ...
        <div>
          ...
          <div className="mt-1 flex flex-wrap items-center gap-1.5">
            {tags.map((tag) => (
              <span
                key={tag}
                className="inline-flex items-center gap-1 rounded-md bg-amber-100/70 px-2 py-0.5 text-xs font-semibold text-amber-900"
              >
                #{tag}
                <button
                  type="button"
            ...
            {tags.length < 5 && (
              <div className="flex items-center gap-1">
                <input
                  ...
                  placeholder="Add tag..."
                  className="w-24 rounded-md border border-hairline bg-surface px-2 py-0.5 text-xs text-ink focus:border-amber-500 focus:bg-white focus:outline-none"
                />
                <button
                  type="button"
                  onClick={addTag}
                  disabled={!tagInput.trim()}
                  className="rounded-md bg-amber-200/80 px-2 py-0.5 text-xs font-bold text-amber-900 disabled:opacity-40"
                >
                  +
                </button>
              </div>
