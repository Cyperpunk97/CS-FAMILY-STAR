...
import StudySpotPanel from './StudySpotPanel';
...
export default function VenueSheet({
  ...
  return (
    ...
    >
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
        ...
        <div className="flex items-start gap-3 px-5 pb-4 pt-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            */}
            ...
            <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs">
              ...
              {venue.isOnCampus && (
                <span className="rounded-md border border-emerald-200 bg-emerald-50 px-1.5 py-0.5 font-bold text-emerald-800">
          ...
          <button
            ...
            onClick={onClose}
            aria-label="Close"
            className="-me-1 -mt-1 shrink-0 rounded-xl p-2 text-ink-faint transition hover:bg-ink/5 hover:text-ink"
          >
            <X className="h-5 w-5" aria-hidden="true" />
          </button>
        </div>

        {/* Quick actions: Directions, Menu (always available for on-campus spots), Call, Share, Save */}
        <div className="flex gap-2 px-5">
          ...
          <button
            ...
            className="flex flex-1 min-w-[72px] flex-col items-center gap-1 rounded-xl border border-brand-200 bg-brand-50/70 px-2 py-2.5 text-xs font-bold text-brand-800 transition hover:bg-brand-100 hover:border-brand-300 active:scale-95"
          >
            <UtensilsCrossed className="h-4 w-4 text-brand-600" aria-hidden="true" />
            {t('venue.menu')}
          </button>
        ...
        <dl className="mx-5 mt-4 grid grid-cols-2 gap-3 rounded-2xl bg-surface p-4 text-sm">
          <div>
            <dt className="flex items-center justify-between text-xs font-bold text-ink-soft">
              ...
              {onChangeFaculty && (
                <button
                  ...
                  className="shrink-0 text-xs font-semibold text-brand-700 hover:text-brand-900 underline decoration-dotted ms-1"
                >
                  {t('venue.change')}
                </button>
              )}
            ...
            <dd className="mt-1 font-bold text-ink">
              ...
              <span className="ms-1 text-xs font-medium text-ink-faint">
          ...
          <div>
            ...
            <dd className="mt-1 font-bold text-ink">
              {price.text}
              <span className="ms-1 text-xs font-medium text-ink-faint">{price.note}</span>
            </dd>
          </div>
        ...
        {/* Dedicated Restaurant Menu & Provide Options Card */}
        <div className="mx-5 mt-3 rounded-2xl border border-brand-200/80 bg-linear-to-r from-brand-50/80 to-amber-50/50 p-3.5 shadow-2xs">
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="flex items-center gap-1.5">
                <UtensilsCrossed className="h-4 w-4 text-brand-700" />
                <h3 className="text-xs font-extrabold text-ink">Restaurant Menu & Dishes</h3>
              </div>
              <p className="mt-1 text-xs text-ink-soft">
                View current menu prices in EGP, student combos, bestsellers, or provide new items.
              </p>
            </div>
            ...
            <button
              type="button"
              onClick={() => setMenuOpen(true)}
              className="shrink-0 flex items-center gap-1 rounded-xl bg-brand-600 px-3 py-1.5 text-xs font-bold text-white shadow-sm transition hover:bg-brand-700 active:scale-95"
            >
              {t('venue.viewMenu')}
            </button>
          </div>
        ...
        {/* What students have been paying over time. Renders itself away when the
            sample is too thin to say anything true. */}
        <div className="mx-5">
          <PriceTrendCard reviews={reviews} />
          <StudySpotPanel venueId={venue.id} />
        </div>
        ...
        {reviews.length > 0 && (
          <section className="mx-5 mt-5" aria-labelledby={`breakdown-${venue.id}`}>
            <h3 id={`breakdown-${venue.id}`} className="mb-2 text-xs font-bold text-ink">
              {t('venue.ratingBreakdown')}
              {venue.reviewCount > reviews.length && (
                <span className="ms-1 font-medium text-ink-faint">
              ...
              {distribution.map(({ star, count }) => {
                ...
                return (
                  <div key={star} className="flex items-center gap-2 text-xs">
                    ...
                    <span className="w-6 shrink-0 text-end font-medium text-ink-faint">{count}</span>
                  </div>
                );
              })}
            </div>
        ...
        <div className="mx-5 mt-5 flex items-center justify-between gap-3 rounded-2xl border border-amber-200/90 bg-gradient-to-r from-amber-50/80 to-orange-50/70 p-3.5 shadow-2xs">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-amber-900/80 truncate">Have photos or happy stories from here?</p>
            </div>
          </div>
          <Link
                ...
                <li key={review.id} className="rounded-2xl bg-surface p-4">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {review.user_name && (
                        <RaterBadge reviewCount={3} className="text-xs py-0 px-1.5" />
                      )}
                    </div>
      ...
      {menuOpen && (
        <MenuModal venue={venue} open={menuOpen} onClose={() => setMenuOpen(false)} />
      )}
    </Modal>
  );
}
