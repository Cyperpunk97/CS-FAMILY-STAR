    ...
    <div className="flex flex-1 flex-col">
      ...
      <div className="mb-6 flex items-center justify-between gap-3 border-b border-amber-200/50 pb-4">
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={() => setIsFueInfoOpen(true)}
            className="group inline-flex items-center gap-1.5 rounded-full border border-amber-300/80 bg-white/90 px-3 py-1.5 text-xs font-bold text-amber-950 shadow-2xs transition hover:border-amber-400 hover:bg-amber-50 active:scale-95"
      ...
      <section className="relative overflow-hidden rounded-3xl border border-amber-200/90 bg-gradient-to-br from-amber-100/70 via-orange-50/60 to-yellow-100/50 p-4 shadow-sm sm:p-6">
        {/* Playful decorative sunshine glows */}
        <div className="pointer-events-none absolute -end-6 -top-6 h-36 w-36 rounded-full bg-amber-300/30 blur-2xl" />
        <div className="pointer-events-none absolute -bottom-6 -start-6 h-36 w-36 rounded-full bg-orange-300/20 blur-2xl" />

        <div className="relative z-10 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="max-w-xl">
            <div className="inline-flex items-center gap-1.5 rounded-full bg-amber-200/80 px-3 py-1 text-xs font-extrabold text-amber-950">
      ...
      <div className="mt-6 space-y-3">
        ...
        <div className="relative">
          <Search className="pointer-events-none absolute start-3.5 top-3 h-4 w-4 text-amber-700/60" />
          <input
            type="search"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search memories by spot, friend, story, or tag..."
            className="w-full rounded-2xl border border-amber-200/80 bg-card py-2.5 ps-10 pe-4 text-xs text-ink placeholder:text-ink-faint shadow-2xs focus:border-amber-500 focus:bg-white focus:outline-none"
          />
          {searchQuery && (
            <button
              type="button"
              onClick={() => setSearchQuery('')}
              className="absolute end-1.5 top-1/2 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full text-xs text-ink-faint hover:text-ink"
            >
              ✕
            </button>
          )}
        ...
        {/* Vibe filter pills */}
        <div className="-mx-4 flex items-center gap-1.5 overflow-x-auto px-4 pb-1 scrollbar-none [scroll-padding-inline:1rem] sm:mx-0 sm:px-0">
          <button
            type="button"
            ...
            {filteredMemories.map((memory, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="pointer-events-none absolute -top-2 start-1/2 -translate-x-1/2 h-3.5 w-16 -rotate-1 rounded-xs bg-amber-200/60 shadow-2xs backdrop-blur-xs border border-amber-300/40" />

                  {/* Top Bar: Mood badge & Date */}
                  <div className="flex items-center justify-between gap-2 pt-1 text-xs">
                    <span
                      className={`inline-flex min-w-0 items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-bold ${moodInfo.bgClass} ${moodInfo.textClass}`}
                    >
                      <span aria-hidden="true">{moodInfo.emoji}</span>
                      <span className="truncate">{moodInfo.label}</span>
                    </span>

                    <span className="flex shrink-0 items-center gap-1 text-xs font-medium text-ink-faint">
                  ...
                  {memory.photoUrl ? (
                    <div className="mt-3 overflow-hidden rounded-xl border-4 border-white bg-amber-50 shadow-inner">
                      ...
                      {memory.photoCaption && (
                        <p className="bg-amber-50/90 px-2 py-1 text-center font-mono text-xs text-amber-900/80 italic">
                          {memory.photoCaption}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="mt-3 flex aspect-[16/6] w-full items-center justify-center rounded-xl border border-dashed border-amber-200 bg-gradient-to-r from-amber-50 to-orange-50/50 p-3 text-center">
                      ...
                      <p className="ms-2 font-serif text-xs italic text-amber-900/70">
                  ...
                  <div className="mt-3">
                    <h2 className="text-base font-bold text-ink break-words hyphens-auto group-hover:text-amber-950">
                      {memory.title}
                    </h2>

                    {/* Venue tag with direct link to Food Guide if attached */}
                    <div className="mt-1 flex min-w-0 items-center gap-1.5 text-xs text-amber-800 font-semibold">
                      <MapPin className="h-3.5 w-3.5 text-amber-600 shrink-0" />
                      <span className="min-w-0 truncate">{memory.venueName}</span>

                      {memory.venueId && (
                        <Link
                          href={`/?spot=${encodeURIComponent(memory.venueId)}`}
                          className="ms-auto inline-flex shrink-0 items-center gap-0.5 rounded-md bg-amber-100/60 px-1.5 py-0.5 text-xs font-bold text-amber-900 hover:bg-amber-200 transition"
                  ...
                  {/* Story */}
                  <p className="mt-2.5 whitespace-pre-line break-words text-xs leading-relaxed text-ink-soft">
                    {memory.story}
                  </p>
                  ...
                  {memory.tags && memory.tags.length > 0 && (
                    <div className="mt-3 flex flex-wrap gap-1">
                      {memory.tags.map((t) => (
                        <span
                          key={t}
                          className="max-w-full truncate rounded-md bg-surface-sunken px-1.5 py-0.5 text-xs font-medium text-ink-faint"
                        >
                          #{t}
                        </span>
                      ))}
                  ...
                  <div className="mt-4 flex items-center justify-between gap-2 border-t border-hairline/60 pt-3 text-xs">
                    <div className="flex min-w-0 flex-col">
                      <span className="truncate text-xs font-bold text-ink">{memory.authorName}</span>
                      {memory.faculty && (
                        <span className="truncate text-xs text-ink-faint" title={memory.faculty}>
                          {memory.faculty}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex shrink-0 items-center gap-1.5">
                      ...
                      <button
                        ...
                        className="relative flex h-11 w-11 items-center justify-center rounded-lg text-ink-faint transition hover:bg-amber-50 hover:text-amber-800"
                        aria-label="Copy link to memory"
                      ...
                      >
                        ...
                        {isCopied && (
                          <span className="pointer-events-none absolute -top-6 end-0 whitespace-nowrap rounded bg-ink px-1 py-0.5 text-xs font-bold text-white shadow-xs">
                            {t('venue.copied')}
                          </span>
                        )}
                      </button>
                      ...
                      <button
                        ...
                        className={`flex min-h-11 items-center gap-1 rounded-full px-3 py-1 text-xs font-bold transition ${
                          isCheered
