...
/**
 * Cache-only lookup. Never touches the network, never returns a promise.
 *
 * The page render used to `await fetchWikipediaLogo` once per venue. The cache made
 * that free on a warm instance, but on a cold start it was one outbound Wikipedia
 * request per logo-less worldwide brand before the first byte of HTML — each with a
 * 6 second timeout, so a slow Wikipedia delayed the whole page for every visitor.
 *
 * A brand's logo URL is stable, so the render path now takes whatever is already
 * cached and moves on. Anything missing falls back to the monogram, which is the
 * same thing the user would have seen if the fetch had failed.
 */
export function getCachedWikipediaLogo(brand: string): WikipediaLogoInfo | null {
  if (!isWorldwideRestaurant(brand)) return null;

  const cached = LOGO_CACHE.get(brand);
  if (cached !== undefined) return cached;

  // A brand with a verified local file needs no network call at all, so it can be
  // resolved synchronously on the first request too.
