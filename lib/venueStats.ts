...
export async function getVenuesWithStats(): Promise<VenueWithStats[]> {
  ...
  return VENUES.map((venue) => {
    ...
    // Cache-only: never blocks the render on an outbound Wikipedia request.
    // A miss falls back to the brand monogram, which is a fine answer.
    if (!logoUrl) {
      const wikiInfo = getCachedWikipediaLogo(venue.brand);
      if (wikiInfo) {
