 ...
 *
 * This is deliberately bounded. The store previously accepted an unlimited number
 * of memories, each carrying up to 5 MB of base64 image inside `photoUrl`, and never
 * evicted anything — a few hundred POSTs were enough to exhaust the instance's heap.
 * Photos now live in Supabase Storage and only their URL is kept here, and the list
 * is capped, so the worst case is bounded by `MAX_MEMORIES * ~1.5 KB`.
 *
 * It is still per-instance and still lost on restart. That is a known limitation,
 * not a design: memories need a real table (see supabase/schema.sql).
 */
...
export function addInMemoryMemory(memory: Omit<Memory, 'id' | 'createdAt' | 'cheersCount'>): Memory {
  ...
  inMemoryMemories = [newMemory, ...inMemoryMemories].slice(0, MAX_MEMORIES);
  return newMemory;
}
