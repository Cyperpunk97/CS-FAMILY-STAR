 ...
 * Without this check a client could attach any URL to a review or a memory, turning
 * every reader's browser into a request to a third-party server of the attacker's
 * choosing — which hands that server every reader's IP address and user agent.
 *
 * Returns `undefined` for "invalid", which is distinct from `null` for "no image".
 */
export function validateImageUrl(input: unknown): string | null | undefined {
  if (input === null || input === undefined || input === '') return null;
  if (typeof input !== 'string') return undefined;

  // A base64 `data:` URL is not a link, it is the whole image inline. Accepting one
  // means holding megabytes of caller-supplied data in the server's memory.
